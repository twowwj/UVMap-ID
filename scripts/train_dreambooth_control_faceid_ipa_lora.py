...
def set_ip_adapter_(unet, num_tokens=16, scale=0.5):
    attn_procs = {}
    ...
    state_dict = torch.load('/workspace/wwang/ControlNetDreamBooth-main/id_adapter/models/ip-adapter-faceid_sd15.bin', map_location="cpu")
...
def log_validationv2(vae, text_encoder, tokenizer, unet, controlnet, args, accelerator, weight_dtype, 
    ...
    with torch.no_grad():
        ...
        latents = randn_tensor(shape, device=accelerator.device)
        ...
        text_embeddings, uncond_embeddings = encode_prompt(
            prompt, tokenizer, text_encoder, accelerator.device, do_classifier_free_guidance, n_prompt
        )
        
        faceid_embeds = validation_image_embed.to(accelerator.device, dtype=unet.dtype)
        ...
        with torch.no_grad():
            for i, t in enumerate(timesteps):
                ...
                if do_classifier_free_guidance:
                    ...
                    noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_ - noise_pred_uncond)
                
                latents = noise_scheduler.step(noise_pred, t, latents).prev_sample 

       # Post-processing
        image = vae.decode(latents / 0.18215, return_dict=False)[0]
        image_processor = VaeImageProcessor(vae_scale_factor=vae_scale_factor, do_convert_rgb=True, do_normalize=True)
...
def main(args):
    ...
    load_ip_adapter(ip_ckpt, unet, image_proj_model)
    
    # image_proj_model.load_state_dict(torch.load('/workspace/wwang/ControlNetDreamBooth-main/ip_adapter/models/ip-adapter-faceid_sd15.bin')['image_proj'])
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                text_embeddings = encode_prompt(
                    [args.instance_prompt] * args.train_batch_size, 
                    tokenizer, 
                    text_encoder, 
                    accelerator.device, 
                    do_classifier_free_guidance, 
                    [args.negative_instance_prompt] * args.train_batch_size
                    )
                ...
                if random.random() < 0.1:
                    prompt_image_emb = torch.zeros_like(prompt_image_emb)
